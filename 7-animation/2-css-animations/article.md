...
У `transition-duration` вказуємо, скільки часу має займати анімація. Час має бути в [форматі часу CSS](http://www.w3.org/TR/css3-values/#time): в секундах `s` або мілісекундах `ms`.
...
Якщо ви зараз натисните на цифру - анімація почеться з поточної секунди:
...
Ця властивість приймає два види значень: крива Безьє або кроки. Почнемо з кривої, оскільки вона використовується частіше.

### Крива Безьє

Функцію хронометражу можна встановити як [криву Безьє](/bezier-curve) з 4 контрольними точками, які задовольняють умовам:
...
Синтаксис кривої Безьє в CSS: `cubic-bezier(x2, y2, x3, y3)`. Тут нам потрібно вказати лише 2-у і 3-ю контрольні точки, тому що 1-а фіксована на `(0,0)`, а 4-а — `(1,1)`.
...
Ми можемо використати іншу криву Безьє: `cubic-bezier(0.0, 0.5, 0.5 ,1.0)`.
...
Контрольні точки на кривій можуть мати будь-які координати `y`: навіть негативні або величезні. Тоді крива Безьє також буде дуже низькою або високою, що змушує анімацію виходити за межі нормального діапазону.
...
Але якщо ви натисните на потяг, ви побачите, що:
...
Чому це відбувається, стає зрозумілим, якщо ми подивимося на графік даної кривої Безьє:
...
Але як зробити криву Безьє для конкретного завдання? Інструментів багато. Наприклад, ми можемо це зробити на сайті <http://cubic-bezier.com/>.
...
Ми можемо бачити, що під час анімації: коли ми клікаємо по цифрі, вона відразу змінюється на `1` (перший крок), а потім змінюється на початку наступної секунди.
